       ...
       <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8 md:gap-12">
                ...
                <main className="md:col-span-3">
                    <h1 className="text-4xl md:text-5xl font-extrabold mb-3 leading-tight tracking-tighter">{game.title}</h1>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-gray-800 pb-4 mb-6">
                        <InfoPill label="Genre" value={game.genre} />
                        <InfoPill label="Category" value={game.category} />
                        <InfoPill label="Release" value={new Date(game.releaseDate).toLocaleDateString()} />
                    </div>

                    <div className="bg-brand-dark p-4 sm:p-6 rounded-lg border border-gray-800 my-8">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <p className="text-3xl font-bold" aria-label={`Price: ${game.price.toFixed(2)} dollars`}>${game.price.toFixed(2)}</p>
                            <div className="flex items-center gap-3">
                                {game.patchUrl && (
                                     <a href={game.patchUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center whitespace-nowrap bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 ease-in-out">
                                        <PatchIcon /> <span>Yama</span>
                                    </a>
                                )}
                                <a href={game.downloadUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center whitespace-nowrap bg-brand-purple hover:bg-violet-500 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 ease-in-out transform hover:scale-105 hover:drop-shadow-[0_4px_18px_rgba(109,40,217,0.5)]">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold mb-4 border-l-4 border-brand-purple pl-3">About This Game</h2>
                    ...
                    {game.screenshots && game.screenshots.length > 0 && (
                        <div className="mt-12">
                            <h2 className="text-2xl font-bold mb-4 border-l-4 border-brand-purple pl-3">Screenshots</h2>
